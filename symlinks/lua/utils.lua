...
function M.JBLRunCurrentTest(mode)
    -- tests.test_api_v1.RejectionReasonTests.test_delete_another_user
    -- apps.companies.tests.test_tasks::EmailSubscribersTest::test_company
    ...
    local j_test_path
    if mode == 'one' then
      j_test_path = filepath .. '::' .. class_name .. '::' .. method_name
      command = 'T ' .. 'jtest_one ' .. j_test_path
    else
      j_test_path = filepath
      command = 'T ' .. 'jtest ' .. j_test_path
    end
